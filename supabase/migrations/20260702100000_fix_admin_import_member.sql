-- Migration: Fix confirmed_at non-DEFAULT insertion error in modern Supabase setups
-- Date: 2026-07-02
...
BEGIN
  ...
  -- Insert into auth.users with all expected standard columns populated (excluding generated column "confirmed_at")
  ...
  IF p_company_name IS NOT NULL AND p_company_name <> '' THEN
    INSERT INTO public.business_directory (
      ...
      v_user_id,
      p_company_name,
      concat('Registered Chamber Member business specializing in ', coalesce(p_membership_type, 'commerce'), '.'),
      p_email,
      nullif(p_phone, ''),
      'Retail',
